...
class ValidationValidatorTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testValidateArray()
    {
        ...
        $v = new Validator(['foo' => new Symfony\Component\HttpFoundation\File\File('/tmp/foo', false)], ['foo' => 'Array']);
        $this->assertFalse($v->passes());
    }
    ...
    public function testValidateRequired()
    {
        ...
        $file = new File('', false);
        $v = new Validator(['name' => $file], ['name' => 'Required']);
        $this->assertFalse($v->passes());
    ...
    public function testValidateRequiredWith()
    {
        ...
        $file = new File('', false);
        $v = new Validator(['file' => $file, 'foo' => ''], ['foo' => 'required_with:file']);
        $this->assertTrue($v->passes());

        $file = new File(__FILE__, false);
        $foo = new File(__FILE__, false);
        $v = new Validator(['file' => $file, 'foo' => $foo], ['foo' => 'required_with:file']);
        $this->assertTrue($v->passes());
    ...
    public function testValidateRequiredWithout()
    {
        ...
        $file = new File('', false);
        $v = new Validator(['file' => $file], ['foo' => 'required_without:file']);
        $this->assertFalse($v->passes());

        $foo = new File('', false);
        $v = new Validator(['foo' => $foo], ['foo' => 'required_without:file']);
        $this->assertFalse($v->passes());
        ...
        $file = new File(__FILE__, false);
        $foo = new File(__FILE__, false);
        $v = new Validator(['file' => $file, 'foo' => $foo], ['foo' => 'required_without:file']);
        $this->assertTrue($v->passes());
    ...
    public function testRequiredIf()
    {
        ...
        $v = new Validator(['foo' => true], ['bar' => 'required_if:foo,false']);
        $this->assertTrue($v->passes());

        $v = new Validator(['foo' => true], ['bar' => 'required_if:foo,true']);
        $this->assertTrue($v->fails());
    ...
    public function testValidateBetween()
    {
        ...
        $file = $this->getMock('Symfony\Component\HttpFoundation\File\File', ['getSize'], [__FILE__, false]);
        $file->expects($this->any())->method('getSize')->will($this->returnValue(3072));
        $v = new Validator([], ['photo' => 'Between:1,5']);
        $v->setFiles(['photo' => $file]);
        $this->assertTrue($v->passes());

        $file = $this->getMock('Symfony\Component\HttpFoundation\File\File', ['getSize'], [__FILE__, false]);
        $file->expects($this->any())->method('getSize')->will($this->returnValue(4072));
        $v = new Validator([], ['photo' => 'Between:1,2']);
        $v->setFiles(['photo' => $file]);
        $this->assertFalse($v->passes());
    }
    ...
    public function testValidateMin()
    {
        ...
        $file = $this->getMock('Symfony\Component\HttpFoundation\File\File', ['getSize'], [__FILE__, false]);
        $file->expects($this->any())->method('getSize')->will($this->returnValue(3072));
        $v = new Validator([], ['photo' => 'Min:2']);
        $v->setFiles(['photo' => $file]);
        $this->assertTrue($v->passes());

        $file = $this->getMock('Symfony\Component\HttpFoundation\File\File', ['getSize'], [__FILE__, false]);
        $file->expects($this->any())->method('getSize')->will($this->returnValue(4072));
        $v = new Validator([], ['photo' => 'Min:10']);
        $v->setFiles(['photo' => $file]);
        $this->assertFalse($v->passes());
    }
    ...
    public function testValidateDistinct()
    {
        ...
        $v = new Validator(['cat' => ['sub' => [['prod' => [['id' => 1]]], ['prod' => [['id' => 2]]]]]], ['cat.sub.*.prod.*.id' => 'distinct']);
        $this->assertTrue($v->passes());

        $v = new Validator(['cat' => ['sub' => [['prod' => [['id' => 2]]], ['prod' => [['id' => 2]]]]]], ['cat.sub.*.prod.*.id' => 'distinct']);
        $this->assertFalse($v->passes());
    ...
    {
        $v = new Validator(['email' => 'foo'], ['email' => 'Unique:users']);
        $mock = m::mock('ConstantNull\Validation\PresenceVerifierInterface');
        ...
        $v = new Validator(['email' => 'foo'], ['email' => 'Unique:connection.users']);
        $mock = m::mock('ConstantNull\Validation\PresenceVerifierInterface');
        ...
        $v = new Validator(['email' => 'foo'], ['email' => 'Unique:users,email_addr,1']);
        $mock = m::mock('ConstantNull\Validation\PresenceVerifierInterface');
        $mock->shouldReceive('setConnection')->once()->with(null);
        ...
        $v = new Validator(['email' => 'foo'], ['email' => 'Unique:users,email_addr,1,id_col']);
        $mock = m::mock('ConstantNull\Validation\PresenceVerifierInterface');
        $mock->shouldReceive('setConnection')->once()->with(null);
        ...
        $v = new Validator(['users' => [['id' => 1, 'email' => 'foo']]], ['users.*.email' => 'Unique:users,email,[users.*.id]']);
        $mock = m::mock('ConstantNull\Validation\PresenceVerifierInterface');
        $mock->shouldReceive('setConnection')->once()->with(null);
        ...
        $v = new Validator(['email' => 'foo'], ['email' => 'Unique:users,email_addr,NULL,id_col,foo,bar']);
        $mock = m::mock('ConstantNull\Validation\PresenceVerifierInterface');
        $mock->shouldReceive('setConnection')->once()->with(null);
    ...
    {
        $v = new Validator([['email' => 'foo', 'type' => 'bar']], ['*.email' => 'unique:users', '*.type' => 'exists:user_types']);
        $mock = m::mock('ConstantNull\Validation\PresenceVerifierInterface');
    ...
    {
        $v = new Validator(['email' => 'foo'], ['email' => 'Exists:users']);
        $mock = m::mock('ConstantNull\Validation\PresenceVerifierInterface');
        $mock->shouldReceive('setConnection')->once()->with(null);
        ...
        $v = new Validator(['email' => 'foo'], ['email' => 'Exists:users,email,account_id,1,name,taylor']);
        $mock = m::mock('ConstantNull\Validation\PresenceVerifierInterface');
        $mock->shouldReceive('setConnection')->once()->with(null);
        ...
        $v = new Validator(['email' => 'foo'], ['email' => 'Exists:users,email_addr']);
        $mock = m::mock('ConstantNull\Validation\PresenceVerifierInterface');
        $mock->shouldReceive('setConnection')->once()->with(null);
        ...
        $v = new Validator(['email' => 'foo'], ['email' => 'Exists:connection.users']);
        $mock = m::mock('ConstantNull\Validation\PresenceVerifierInterface');
        $mock->shouldReceive('setConnection')->once()->with('connection');
        $mock->shouldReceive('getCount')->once()->with('users', 'email', 'foo', null, null, [])->andReturn(true);
        $v->setPresenceVerifier($mock);
    ...
    {
        $v = new Validator(['id' => 'foo'], ['id' => 'Integer|Exists:users,id']);
        $mock = m::mock('ConstantNull\Validation\PresenceVerifierInterface');
        ...
        $v = new Validator(['id' => '1'], ['id' => 'Integer|Exists:users,id']);
        $mock = m::mock('ConstantNull\Validation\PresenceVerifierInterface');
        $mock->shouldReceive('setConnection')->once()->with(null);
    ...
    public function testValidateActiveUrl()
    {
        ...
        $v = new Validator(['x' => ['fdsfs', 'fdsfds']], ['x' => 'active_url']);
        $this->assertFalse($v->passes());
    ...
    public function testValidateImageDimensions()
    {
        ...
        $v = new Validator(['x' => 'file'], ['x' => 'dimensions']);
        $this->assertTrue($v->fails());

        $v = new Validator([], ['x' => 'dimensions:min_width=1']);
        $v->setFiles(['x' => $uploadedFile]);
        $this->assertTrue($v->passes());

        $v = new Validator([], ['x' => 'dimensions:min_width=5']);
        $v->setFiles(['x' => $uploadedFile]);
        $this->assertTrue($v->fails());

        $v = new Validator([], ['x' => 'dimensions:max_width=10']);
        $v->setFiles(['x' => $uploadedFile]);
        $this->assertTrue($v->passes());

        $v = new Validator([], ['x' => 'dimensions:max_width=1']);
        $v->setFiles(['x' => $uploadedFile]);
        $this->assertTrue($v->fails());

        $v = new Validator([], ['x' => 'dimensions:min_height=1']);
        $v->setFiles(['x' => $uploadedFile]);
        $this->assertTrue($v->passes());

        $v = new Validator([], ['x' => 'dimensions:min_height=5']);
        $v->setFiles(['x' => $uploadedFile]);
        $this->assertTrue($v->fails());

        $v = new Validator([], ['x' => 'dimensions:max_height=10']);
        $v->setFiles(['x' => $uploadedFile]);
        $this->assertTrue($v->passes());

        $v = new Validator([], ['x' => 'dimensions:max_height=1']);
        $v->setFiles(['x' => $uploadedFile]);
        $this->assertTrue($v->fails());

        $v = new Validator([], ['x' => 'dimensions:width=3']);
        $v->setFiles(['x' => $uploadedFile]);
        $this->assertTrue($v->passes());

        $v = new Validator([], ['x' => 'dimensions:height=2']);
        $v->setFiles(['x' => $uploadedFile]);
        $this->assertTrue($v->passes());

        $v = new Validator([], ['x' => 'dimensions:min_height=2,ratio=3/2']);
        $v->setFiles(['x' => $uploadedFile]);
        $this->assertTrue($v->passes());

        $v = new Validator([], ['x' => 'dimensions:ratio=1/1']);
        $v->setFiles(['x' => $uploadedFile]);
        $this->assertTrue($v->fails());
    }
    ...
    public function testValidateMimetypes()
    {
        ...
        $file = $this->getMock('Symfony\Component\HttpFoundation\File\UploadedFile', ['guessExtension'], $uploadedFile);
        $file->expects($this->any())->method('guessExtension')->will($this->returnValue('php'));
        $v = new Validator([], ['x' => 'mimetypes:text/x-php']);
        $v->setFiles(['x' => $file]);
        $this->assertTrue($v->passes());
    }
    ...
    public function testValidateFile()
    {
        ...
        $v = new Validator(['x' => '1'], ['x' => 'file']);
        $this->assertTrue($v->fails());

        $v = new Validator([], ['x' => 'file']);
        $v->setFiles(['x' => $file]);
        $this->assertTrue($v->passes());
    }
...
1'], ['x' => 'Alpha']);
    ...
    public function testValidateDateAndFormat()
    {
        ...
        $v = new Validator(['x' => '1325376000'], ['x' => 'date']);
        $this->assertTrue($v->fails());
        ...
        $v = new Validator(['x' => new DateTime()], ['x' => 'date']);
        $this->assertTrue($v->passes());
        ...
        $v = new Validator(['x' => ['Not', 'a', 'date']], ['x' => 'date_format:Y-m-d']);
        $this->assertTrue($v->fails());
    }
    ...
    public function testBeforeAndAfter()
    {
        ...
        $v = new Validator(['x' => new \DateTime('2000-01-01')], ['x' => 'Before:2012-01-01']);
        $this->assertTrue($v->passes());
        ...
        $v = new Validator(['x' => new \DateTime('2012-01-01')], ['x' => 'After:2000-01-01']);
        $this->assertTrue($v->passes());
        ...
        $v = new Validator(['start' => new DateTime('2012-01-01'), 'ends' => new \DateTime('2013-01-01')], ['start' => 'Before:ends', 'ends' => 'After:start']);
        $this->assertTrue($v->passes());

        $v = new Validator(['start' => '2012-01-01', 'ends' => new DateTime('2013-01-01')], ['start' => 'Before:ends', 'ends' => 'After:start']);
        $this->assertTrue($v->passes());

        $v = new Validator(['start' => new DateTime('2012-01-01'), 'ends' => new DateTime('2000-01-01')], ['start' => 'After:2000-01-01', 'ends' => 'After:start']);
        $this->assertTrue($v->fails());

        $v = new Validator(['start' => 'today', 'ends' => 'tomorrow'], ['start' => 'Before:ends', 'ends' => 'After:start']);
        $this->assertTrue($v->passes());
    }
    ...
    public function testSometimesAddingRules()
    {
        ...
        $v = new Validator(['foo' => [['name' => 'first', 'title' => null]]], []);
    ...
    {
        $data = ['names' => [['second' => []]]];
        $v = new Validator($data, ['names.*.second' => 'sometimes|required']);
        $this->assertFalse($v->passes());
    }
    ...
    public function testValidateImplicitEachWithAsterisksForRequiredNonExistingKey()
    {
        ...
        $v = new Validator($data, ['companies.*.name' => 'required']);
        $this->assertFalse($v->passes());
    ...
    {
        $v = new Validator(['foo' => [1, 2, 3]], ['foo' => 'size:4']);
        $this->assertFalse($v->passes());

        $v = new Validator(['foo' => [1, 2, 3, 4]], ['foo' => 'size:4']);
        $this->assertTrue($v->passes());

        $v = new Validator(['foo' => [['bar' => [1, 2, 3]], ['bar' => [1, 2, 3]]]], ['foo.*.bar' => 'size:4']);
        $this->assertFalse($v->passes());
    ...
    public function testExtractDataFromPath()
    {
        ...
        $v = new Validator([['email' => 'mail'], ['email' => 'mail2']], []);
        $this->assertEquals([['email' => 'mail'], ['email' => 'mail2']], $method->invoke($v, null));

        $v = new Validator(['cat' => ['cat1' => ['name']], ['cat2' => ['name2']]], []);
        $this->assertEquals(['cat' => ['cat1' => ['name']]], $method->invoke($v, 'cat.cat1'));

        $v = new Validator(['cat' => ['cat1' => ['name' => '1', 'price' => 1]], ['cat2' => ['name' => 2]]], []);
    ...
    {
        $v = new Validator(['foo' => ['a', 'b', 'c']], ['foo.*' => 'string']);
